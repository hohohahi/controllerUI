...
@Service
public class DownloadTemplateCommandParser extends AbstractBaseBean implements ICommandParser {
	@Autowired
	...
	public RxTxResponseVO run(byte aid, byte[] dataArea) {
		...
		if (aid == commandType.getAid_Success()) {
			...
			message.setSubMessageType(SubMessageTypeEnum._SubMessageType_AdminPanel_Tab_TemplateManagementPanel_DownloadTemplateButton_);
