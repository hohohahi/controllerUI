...
@Service
public class LearnTemplateCommandParser extends AbstractBaseBean implements ICommandParser {
	@Autowired
	private ITemplateParser templateParser;
	
	@Autowired
	private IProductionDataManager dataManager;
	...
	public RxTxResponseVO run(byte aid, byte[] dataArea) {
		...
		if (aid == commandType.getAid_Success()) {
			...
			final TemplateVO template = templateParser.getTemplateFromByteArray(dataArea);
			dataManager.addLearnedTemplate(template);
			
			final MessageVO message = new MessageVO();
			message.setMessageSource(MessageSourceEnum._MessageSource_AdminPanel_Tab_TemplateManagementPanel_);
			message.setSubMessageType(SubMessageTypeEnum._SubMessageType_AdminPanel_Tab_TemplateManagementPanel_UpdateLearnedTemplate_);
